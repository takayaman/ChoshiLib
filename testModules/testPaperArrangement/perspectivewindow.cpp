    ...
    if( m_textures.size() > 0 ){
        for( int i = 0; i < (int)m_textures.size(); i++ ){
            glPushMatrix();
            {
                ...
                glBegin( GL_QUADS );
                {
                    glTexCoord2d(0.0, 1.0); glVertex2d(-normX,normY);
                    glTexCoord2d(0.0, 0.0); glVertex2d(-normX,-normY);
                    glTexCoord2d(1.0, 0.0); glVertex2d(normX,-normY);
                    glTexCoord2d(1.0, 1.0); glVertex2d(normX,normY);
                }
                glEnd();
            }
            glPopMatrix();
