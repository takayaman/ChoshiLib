...
bool ChoLabeling::init( BaseChoImgProcParam *param )
{
    ...
    if( typeid( *param ) != typeid( ChoLabelingParam ) ){
        LOG(INFO) << "Irregal argument!! *param should be ChoLabelingParam*" << std::endl;
        return false;
    }
...
{
    if( !m_param ){
        LOG(INFO) << "ChoLabelingParam* is unset!!"
                  << "Call init(ChoLabelingParam*)!!" << std::endl;
        return false;
    }
...
ImgProcColorMode ChoLabeling::convertColor( ConvertColorMode eMode )
{
    if( eMode >= CONVERTMODE_NUM ){
        LOG(INFO) << "Argument eMode is invalid." << std::endl;
