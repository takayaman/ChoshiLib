...
{
    CVVEC_CHOSHI retVal; retVal.zeros();
    ...
    for( int iter = 0; iter < termCrit.maxCount; iter++ ){
        ...
        bool stopFlag = false;
        /* define window size */
        minX = cvRound( (double)meanX - sRadius ); minX = MAX( minX, 0 );
        minY = cvRound( (double)meanY - sRadius ); minY = MAX( minY, 0 );
        maxX = cvRound( (double)meanX + sRadius ); maxX = MIN( maxX, m_param->getImageWidth() - 1 );
        maxY = cvRound( (double)meanY + sRadius ); maxY = MIN( maxY, m_param->getImageHeight() - 1 );
